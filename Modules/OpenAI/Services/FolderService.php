...
class FolderService
{
        ...
        return FolderItem::where('folder_items.folder_id', $id)
                    ...
                    $join->on('folder_items.item_id', '=', 'archives.id')
                        ->where(function ($query) {
                            ...
                            ->orWhere(function ($subQuery) {
                                $subQuery->where('folder_items.item_type', 'image_variant')
                ...
                ->leftJoin('archives_meta', function ($join) {
                    $join->on('archives.id', '=', 'archives_meta.owner_id')
                        ->where(function ($query) {
                            $query->where('archives_meta.key', 'image_creator_id')
                ...
                ->selectRaw('
                    CASE
                        ...
                        WHEN speeches.id IS NOT NULL THEN ?
                ...
                ->selectRaw('COALESCE(contents.id, codes.id, speeches.id, audios.id, archives.id) as id')
                ->selectRaw('COALESCE(contents.title, codes.promt, speeches.content, audios.prompt, archives.title) as name')
                ->selectRaw('COALESCE(contents.slug, codes.slug, audios.slug,
                    (
                ...
                ->selectRaw('COALESCE(contents.created_at, codes.created_at, speeches.created_at, audios.created_at, archives.created_at) as created_at')
                ->selectRaw('COALESCE(contents.updated_at, codes.updated_at, speeches.updated_at, audios.updated_at, archives.updated_at) as updated_at')
                ->get();
    }
    ...
    protected function allContents($userId, $folderId, $skipItems) {
        ...
        $images = Archive::where('type', 'image_variant')->whereNull('user_id')
        ->select(
            'archives.id',
        ...
        return $contents->union($images)->union($codes)->union($speeches)->union($audios)->union($archives)
                ->get()
    ...
    protected function addFolderToZip($folder, $zip, $parentFolder = '') {
        ...
        foreach ($folder->folderItem as $item) {
            ...
            if ($data) {
                ...
                if ($data['type'] == 'image_variant' || $data['type'] == 'audio') {
    ...
    protected function getItem($item)
    {
        switch ($item->item_type) {
            ...
            case 'image_variant':
                $image = Archive::where('id', $item->item_id)->first();
                return [
                    'type' => $item->item_type,
                    'fileName' => filterDownloadName($image->title, $image->original_name),
                    'file' => $image->imageUrl(),
                ];
            case 'long_article':
    ...
    public function filterAllData($folders) {
        ...
        $itemTypes = ['document', 'image', 'code', 'speech_to_text_chat_reply', 'audio', 'long_article', 'image_variant'];
        $items = [];
